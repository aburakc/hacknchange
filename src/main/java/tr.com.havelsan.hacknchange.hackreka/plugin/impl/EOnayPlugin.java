...
public class EOnayPlugin extends IAPlugin {
    ...
    public Map<String, Object> run(List<PluginParam> parameters, Map<String,Object> dataMap) {
        ...
        Task task = MockData.getTaskList("tdemir", 1,EnumIntegration.EONAY).get(0);
        task.setTitle("[Bilgi İletişim Teknolojileri] (Havva YILDIRIM) [Paraf]");
        task.setDescription("Konu : İşe alım onayı");
        taskList.add(task);
        task = MockData.getTaskList("tdemir", 1,EnumIntegration.EONAY).get(0);

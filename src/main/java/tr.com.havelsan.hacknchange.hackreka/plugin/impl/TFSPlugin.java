...
public class TFSPlugin extends IAPlugin {
    ...
    public Map<String, Object> run(List<PluginParam> parameters, Map<String,Object> dataMap) {
        ...
        Task task = MockData.getTaskList("tdemir", 1,EnumIntegration.TFS).get(0);
        task.setTitle("[Test Birim] (Test Kullanici) TFS test islemi");
        task.setDescription("TFS Test Detay");
        taskList.add(task);
        task = MockData.getTaskList("tdemir", 1,EnumIntegration.TFS).get(0);
        task.setTitle("[Test Birim] (Test Kullanici) TFS test islemi");
        task.setDescription("TFS Test Detay");
        taskList.add(task);
        task = MockData.getTaskList("tdemir", 1,EnumIntegration.TFS).get(0);
